...
u16 Get_Distance(Ultrasonic_Number_type u)
{
	...
	u16 distance=0;
	//set callback
	Timer1_SetCallBack(TIMER1_ICU,Measure_ISR);
	Timer1_SetCallBack(TIMER1_OV,Timer_Ov_ISR);
	//enable interrupts.
	Timer1_ICP_Mode(RISING);
	Timer1_InterruptEnable(TIMER1_ICU);
	Timer1_InterruptEnable(TIMER1_OV);
...
static void Measure_ISR(void)
{
	if(0 == Flag)
	{
		Time1 = ICR1;
		Count=0;
		Timer1_ICP_Mode(FALLING);
		Flag=1;
	}
	else if(1 == Flag)
	{
		Time2=ICR1;
		Timer1_InterruptDisable(TIMER1_ICU);
		Timer1_InterruptDisable(TIMER1_OV);
		Flag=2;
	}
	else{;}
}
